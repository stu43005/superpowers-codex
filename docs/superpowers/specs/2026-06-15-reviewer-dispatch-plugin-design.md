...
- `task`：對應 Type A（有 prompt body）。讀 `--prompt` 指向的 sidecar，套用所有
  `--set` 注入，寫入 temp 檔，呼叫 `node <companion> task --prompt-file <temp>`。implementer
  report 經**專屬旗標** `--report-file` 傳入（不走 generic `--set`，見 §6），dispatch.sh 以
  其值填入 prompt 的 `[REPORT_FILE_PATH]`。
...
- **刪除時機綁定「dispatch 完成」**：orchestrator 只在該次（背景）dispatch **完成通知**之後
  才刪除報告檔。由於 dispatch.sh 內部對 codex 的呼叫是同步阻塞（跑完才回傳），「dispatch
  完成」即代表 reviewer 已讀畢，因此**不存在早刪競態**，且刪除的永遠是 orchestrator 自己
  建立、可追蹤的唯一檔案。
