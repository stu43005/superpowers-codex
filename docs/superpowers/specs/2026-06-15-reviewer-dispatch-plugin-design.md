...
- **SKILL.md 內建防呆守衛（必要但不充分）**：呼叫 dispatch.sh 前，先檢查解析後的路徑是否仍含
  **字面 `${CLAUDE_PLUGIN_ROOT}`**（代表非 plugin 安裝、inline 展開未發生）或 `dispatch.sh`
  不存在／不可執行；若是，立即以清楚訊息中止並指引補救。
  **侷限（務必正視）**：此守衛只在「**plugin 版 SKILL.md 確實被載入**」時才會執行。若舊的
  skills-collection 副本在發現順序上勝出而**遮蔽**了 plugin 版，Claude 載入的是舊 SKILL.md，
  此守衛根本不會跑。因此遮蔽情境**不能**只靠此守衛，必須靠下面兩項「skill 路徑之外」的機制。
- **發現順序實測與安裝驗證（skill 路徑之外、可強制）**：plan 階段須**實測**確定
  `~/.claude/skills/<name>`、`~/.agents/skills/<name>`、plugin cache 三者的發現/優先順序
  （不得臆測），並寫入文件。安裝/升級流程須含一個 **preflight 檢查**：掃描已知的 legacy
  skill 位置，**若存在任何會遮蔽 plugin 副本的 legacy 路徑，安裝驗證即視為失敗**並要求先移除。
...
- **報告檔由 dispatch.sh 複製成私有副本、reviewer 正確性脫離 orchestrator 通知（演進到終局）**：
  ...
  外部檔；並行天然唯一。
- **遷移防呆移到 skill 路徑之外（vs 只靠 plugin SKILL.md 內守衛）**：第 4 輪指出舊副本遮蔽時
  新 SKILL.md 根本不載入、內建守衛跑不到。改為：實測並文件化發現順序、安裝 preflight 掃描
  legacy 路徑、遮蔽即令安裝驗證失敗並要求移除。內建守衛保留為第二層（plugin SKILL.md 有載入時）。
