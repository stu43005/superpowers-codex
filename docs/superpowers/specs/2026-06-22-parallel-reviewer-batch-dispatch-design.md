...
- **強制讀者協定（雜湊驗證 generation，使並行 republish 下證據一致）**：因 output 檔名在同輪
  重跑會被原子取代（沿用使用者指定的 `<reviewer>.<round>.output.md`、不加 run-id），consumer
  讀取 durable 證據時**必須**：
...
3. **發佈（全有全無、對同輪重跑可還原，僅在「完成批次」時）**：彙整完成**之後**，才依
   ...
   - **完成批次 → 發佈（manifest 為原子提交標記）**：此為**發佈臨界區**，期間延遲 INT/TERM
     ...
     1. **先在 staging 內備妥 manifest（含每個 output 的內容雜湊）**：對每個 job 的 stdout
        staging 檔算 **sha256**，把本輪 manifest 內容（每個 label → canonical 檔名 → **sha256** →
        verdict，皆非 ERROR）寫到 staging 的 `manifest.staged` 並**就地驗證**（格式完整、列出的
        label 與檔案齊備）。雜湊取自 staging 檔，而 sub-step 2 的 `mv` 保留位元組，故已發佈 output
        的雜湊**恆等於** manifest 記載，供 §4.3 讀者協定驗證 generation。驗證失敗 → 視為發佈失敗，
        走「非完成處置」。
...
- **雜湊驗證讀者協定（並行 republish 一致）**：模擬讀者持舊 manifest，在 republish 把某 output
  ...
  （§4.3 讀者協定）。manifest 每筆含正確 sha256。
