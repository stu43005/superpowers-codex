...
3. **發佈（全有全無、對同輪重跑可還原，僅在「完成批次」時）**：彙整完成**之後**，才依
   ...
   - **完成批次 → 發佈（manifest 為原子提交標記）**：此為**發佈臨界區**，期間延遲 INT/TERM
     ...
     1. **先在 staging 內備妥 manifest**：把本輪 manifest 內容（每個 label → canonical 檔名 →
        verdict，皆非 ERROR）寫到 staging 的 `manifest.staged` 並**就地驗證**（格式完整、列出
        的 label 與檔案齊備）。驗證失敗 → 視為發佈失敗，走「非完成處置」。
...
- **同輪重跑 copy-備份 + 原子覆寫**（vs move-備份）：canonical 路徑發佈期間始終存在、原子取代，
  消除「缺檔視窗」；殘餘的「lock-less 並行讀者在 output 已換、manifest 未提交的短窗看到不一致」
  僅在單一 agent 循序模型外才發生，列為已知限制（見 §12）。
...
2. **同輪重跑期間 lock-less 並行讀者的短窗不一致**：已用 copy-備份 + 原子覆寫消除「缺檔視窗」，
   但 output 已原子換、manifest 尚未提交的數毫秒短窗內，未取鎖的並行讀者可能看到「新 output +
   舊 manifest」。**完全消除方式**：放寬 (i)（output 檔名加 run-id 做不可變化、僅原子提交 manifest
   指標）。單一 agent 循序審查迴圈下不存在此並行讀者，故未放寬前列為已知限制。
