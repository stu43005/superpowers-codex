...
spec-compliance 多傳一個 `--set TASK_HEAD=<sha>`（其值由引擎把 sentinel `@HEAD_AT_START@`
替換為單一擷取的 HEAD，見 §4.6 / §5.3）只是多一個 `--set` 鍵值，dispatch.sh 既有的 `--set`
機制已支援，**仍無需對 dispatch.sh 新增任何旗標**。§4.6 的單一 HEAD 擷取/sentinel 替換/斷言、
§9 的互斥鎖皆在引擎層，與 dispatch.sh 無關。
...
- `batch_run`：執行整個佇列（並行 + 節流），落地中繼檔，組裝並輸出 stdout，回傳
  彙整退出碼。取鎖後**只擷取一次** `HEAD_AT_START`，並把所有 job argv 中的 sentinel
  `@HEAD_AT_START@` 替換為該單一 SHA（供 spec-compliance 釘住 diff 終點，見 §4.6 / §5.3），
  再啟動 job。
...
- **讀者協定機制（雜湊驗證 generation，使並行 republish 下證據一致）**——其適用範圍、強制與
  reader 盤點見 **§4.7 Canonical Reader API**；機制為：因 output 檔名在同輪重跑會被原子取代
  （沿用使用者指定的 `<reviewer>.<round>.output.md`、不加 run-id），consumer 讀取 durable 證據時
  **必須**：
  1. 讀 `<stage>.<round>.manifest.md`，取出其中每個 label → 檔名 → **內容雜湊（sha256）** → verdict；
  2. 讀每個列出的 output 檔，**比對其內容雜湊與 manifest 記載是否相符**；
  3. **任一不符**（代表讀到的 output 已被較新的 republish 原子取代，與手上的 manifest 不同
     generation）→ **視為撞上並行 republish，重讀 manifest 與 outputs 重試**；只在「manifest 與其
     列出的所有 output 雜湊全數一致」時才接受該輪為有效證據。
  此樂觀並行協定在不改檔名方案、不需 reader 取鎖的前提下，保證並行 republish 下讀者**要嘛拿到
  完整的舊 generation、要嘛重試後拿到完整的新 generation，絕不接受新舊混合的證據集**。
...
**核心發佈不變式**：**只有「完成批次」才發佈 canonical + 寫 manifest**——「完成批次」
定義為 **HEAD 自始至終穩定（§4.6）且無任何 job 為 ERROR**（ERROR 依 §4.4 第 2 點＝無
verdict 行且退出非零；code-quality 的 prose 屬退出 0 的正常結果、**非** ERROR，不影響完成
判定）。任何非完成批次（含 ERROR、HEAD 無效、中斷、發佈失敗）一律**不寫 manifest**、
**不以新結果覆寫既有的前一完整輪**；所有結果（含 ERROR 項）仍照常經 stdout 回傳給 caller，
批次非零退出。
如此 **manifest 的存在嚴格等價於「該（stage, round）是一個完整有效的審查集」**，事後讀者/
稽核絕不會把含工具失敗的輪當成完整。

| 情形 | 是否發佈 canonical + manifest | 說明 |
| ---- | ----------------------------- | ---- |
| **reviewer 回 findings**（Issues Found / needs-attention，codex 執行成功 → 退出 0；屬「有 verdict」） | **發佈 + 寫 manifest**（完成批次） | findings 是**正常 verdict**；由 SKILL round loop 依 stdout 處理。批次退出 0。 |
| **單一 job 工具失敗**（無 verdict 行 → `ERROR`） | **非完成 → 不寫 manifest、還原前一完整輪** | 其餘 reviewer 結果照常經 stdout 給 agent；該 job 在 Summary 標 `ERROR`，批次非零退出、caller **重跑整個 wrapper（同一完整 job set）**（見 §4.5：manifest 是整批範圍，無單跑單一 reviewer 的路徑）；durable 證據維持上一個完整輪不被污染。 |
| **批次無效**（§4.6 HEAD 漂移） | **非完成 → 不發佈、不寫 manifest** | 混合快照不可落地；stdout 出現 `BATCH INVALID`。 |
| **訊號中斷（發佈前）** | **非完成 → 不發佈** | shutdown：終止子行程、清 staging、釋鎖（尚未動最終區，無需還原）。 |
| **訊號中斷（發佈臨界區內）** | **非完成 → 還原前一完整輪** | 臨界區延遲訊號；phase-aware shutdown 在清 staging **前先 `rollback()`**（步驟 1/3）。 |
| **發佈步驟失敗**（output `mv` 或 manifest 提交 rename 失敗） | **非完成 → 還原前一完整輪、不留新 manifest** | `rollback()`（見步驟 3）。 |
...
1. **私有 staging（review 目錄下、同檔案系統、讀者忽略）+ shutdown trap**：`batch_run`
   ...
   shutdown 是 **phase-aware** 的——它讀一個 `PUBLISH_PHASE` 狀態旗標，依**任一退出路徑**執行：
   ...
   - 關閉/移除 FIFO 資源；
   - **若 `PUBLISH_PHASE` 已進入「備份已移入 staging、提交未完成」** → **先呼叫步驟 3 的
     `rollback()`**（移除已 `mv` 的部分新 output、從 staging 備份還原舊 output+manifest），
     **再**做清理；否則（尚未動最終區）直接清理。**務必在 `rm -rf` staging 之前完成還原**，
     以免把唯一的備份連同 staging 一起刪掉。
   - `rm -rf` 本次 staging 目錄 `<spec name>/.batch.<run-id>/`；
   - 釋放 §9 的 `.batch.lock`。
   （正常成功路徑由 EXIT 觸發同一 shutdown，此時已提交、備份已丟棄、子行程皆已 reap。）
   此外，**發佈臨界區（步驟 3 的 output `mv` 起、至 manifest 原子 rename 提交止）期間延遲
   INT/TERM**（例如 `trap '' INT TERM` 遮罩或設旗標延後處理），使這段短臨界區不被訊號從中
   撕裂；臨界區內任何失敗都呼叫同一個 `rollback()`，離開臨界區後再處理被延遲的訊號。
2. **彙整（讀私有暫存）**：所有 job 結束後，`batch_run` **讀取私有暫存中的 stdout 檔**
   ...
   污染本次回傳結果**。
3. **發佈（全有全無、對同輪重跑可還原，僅在「完成批次」時）**：彙整完成**之後**，才依
   上述**核心發佈不變式**決定是否發佈：
   - **完成判定**：批次未被 §4.6 判為無效（HEAD 自始至終穩定）**且無任何 job 為 ERROR**
     （§4.4 第 2 點定義；prose 非 ERROR）。任一條件不滿足 → **視為非完成**，跳到下方
     「非完成處置」。
   - **先備份（copy，非 move）**：若該（stage, round）的 canonical 檔/`<stage>.<round>.manifest.md`
     已存在（同輪重跑），先把**整組**既有檔 **copy**（非搬移）到本次 staging 的備份區。用
     copy 而非 move，使既有 canonical 路徑在發佈過程中**始終存在**（不會出現「舊檔已移走、新檔
     未到」的消失視窗）；備份供 `rollback()` 還原。
   - **完成批次 → 發佈（manifest 為原子提交標記）**：此為**發佈臨界區**，期間延遲 INT/TERM
     （見步驟 1 phase-aware shutdown）：
     1. **先在 staging 內備妥 manifest（含 schema-version 與每個 output 的內容雜湊）**：對每個 job
        的 stdout staging 檔算 **sha256**，把本輪 manifest 內容寫到 staging 的 `manifest.staged`：
        頂層含 **`schema-version`**（目前 `1`，供不相容讀者 fail closed，見 §4.7）；逐項記
        每個 label → canonical 檔名 → **sha256** → verdict（皆非 ERROR）。並**就地驗證**（格式
        完整、列出的 label 與檔案齊備）。雜湊取自 staging 檔，而 sub-step 2 的 `mv` 保留位元組，
        故已發佈 output 的雜湊**恆等於** manifest 記載，供 §4.7 讀者協定驗證 generation。驗證失敗
        → 視為發佈失敗，走「非完成處置」。
     2. **發佈 output（原子覆寫，永不消失）**：把每個 job 的 stdout staging 檔以
        **same-filesystem 原子 `mv`** rename 到 `<label>.<round>.output.md`。`rename(2)` 對既有檔
        是**原子取代**，故同名 canonical 路徑要嘛是舊內容、要嘛是新內容，**永不出現缺檔或
        torn 檔**（配合上一步 copy-備份，舊檔在覆寫前也一直在）。
     3. **提交點＝manifest 的原子 rename**：output 全數就位後，把已驗證的 `manifest.staged` 以
        **same-filesystem 原子 `mv`** rename 成 `<stage>.<round>.manifest.md`。**此單一 rename
        即本輪的提交點**——在它成功之前該輪不算完整（無/舊 manifest）；成功之後該輪即完整。
        因 manifest 是先在 staging 寫好再原子 rename，讀者**永不會看到半寫/損毀的 manifest**。
     4. 提交成功 → 丟棄備份。
     讀者僅在對應 manifest 存在、且其列出的檔齊備時，才把該輪視為完整有效集。
   - **非完成處置（ERROR / 無效 / 發佈中途任一步失敗，含 manifest 提交失敗）→ 整輪還原、
     不留新 manifest**：`rollback()` 例行程序：(a) **移除本輪已 `mv` 成功的部分新 output 檔**；
     (b) 把備份區的**整組舊 output 與舊 manifest 原封還原**，使前一個已 commit 的完整輪
     **完全保持原狀**（durable 證據不被工具失敗或半套結果污染）；(c) **不留下新 manifest**；
     以非零退出回報。因提交點是 manifest 的單一原子 rename，要嘛舊 manifest 仍在（提交未發生）、
     要嘛新 manifest 已就位（提交完成），**絕不**出現「新 manifest 指向缺檔」「舊 manifest 配
     被覆寫的 ERROR 檔」或「manifest 認證含 ERROR 的輪」之類的混合態。非完成批次的全部結果
     （含 ERROR 項）仍經 stdout 回傳，caller 依 §4.5 重跑。（HEAD 無效與訊號中斷屬非完成的
     子情形：前者於彙整前即判定、後者由步驟 1 phase-aware shutdown 在清理前先跑 `rollback()`，
     兩者都不留新 manifest，與上述一致。）

如此：讀者永不會看到寫到一半的檔；invalid/中斷的執行**不留下讀者可見的證據**（staging
是 `.` 開頭、讀者忽略，且由 shutdown `rm -rf`、子行程被 reap）；同輪重跑發佈失敗時前一個
有效輪原樣保留；發佈用 same-filesystem 原子 rename。**「不混合」對承諾遵循 §4.3 雜湊驗證讀者
協定的並行讀者亦成立**——即使在 output 已換、manifest 未提交的短窗讀取，雜湊不符會觸發重試，
故讀者最終取得的必是單一一致 generation（已提交的舊或新），不會接受混合集。並行/重跑假設、
互斥鎖與邊界見 §9。
...
2. 若**無**此類 verdict 行**且退出碼非零** → `ERROR (tool failed, exit <rc>)`（工具失敗，
   在其 output 區段附 stderr 摘要）——這才是需要 caller **重跑整個 wrapper** 的情形（§4.5）。
...
- **兩種「需要動作」要分清**：(a) **ERROR（工具失敗）** → caller **重跑整個 wrapper**（環境/
  版本/暫時性錯誤，非審查結果）；(b) **reviewer findings**（Issues Found / needs-attention）→
  caller 依 round loop **修正後 re-review**。批次非零退出只對應 (a)；(b) 由 stdout 的 verdict
  行驅動，與退出碼無關。
- **ERROR 復原＝整批重跑（manifest 是整批範圍，無單跑單一 reviewer 的路徑）**：manifest 認證
  的是「該 wrapper 本輪**登記的整組 reviewer**都來自同一 snapshot 且皆有真實 verdict」。因此
  任一 reviewer ERROR 後，**必須以相同的完整 job set 重跑整個 wrapper**——固定 wrapper
  （brainstorm/impl）本就每次都跑固定兩個 reviewer；review-plan 重跑時必須帶**與失敗那輪
  相同**的 `--task`/`--coverage` 集合，**不得**只重跑失敗子集而發出只涵蓋子集的 manifest。
  引擎不提供 per-job 局部重跑/合併（那會讓不同 snapshot 的結果混進同一 manifest）。
...
2. **引擎為 HEAD 的唯一擷取點（消除 wrapper/引擎雙擷取的 TOCTOU）**：若在 git repo 內，
   `batch_run` 在**取得 §9 互斥鎖之後、啟動任何 job 之前**，**只擷取一次**
   `HEAD_AT_START=$(git rev-parse HEAD)`，作為本批次唯一的快照基準。**wrapper 不得自行
   `git rev-parse HEAD`**——避免「wrapper 抓一次、引擎再抓一次，兩次之間 HEAD 變動」導致
   spec-compliance 釘舊 HEAD、code-quality 讀新 HEAD 卻仍通過 start/end 檢查的競態。
3. **以 sentinel 把同一個 HEAD 注入需釘住的 job**：需要釘住 diff 終點的 job（spec-compliance）
   在 `batch_add` 時以 **sentinel** `--set TASK_HEAD=@HEAD_AT_START@` 登記；引擎在步驟 2
   擷取 `HEAD_AT_START` 後、啟動 job 前，把所有 job argv 中的 `@HEAD_AT_START@` 替換為該
   單一 SHA。如此 spec-compliance 釘住的終點 **恆等於** 引擎基準 `HEAD_AT_START`；
   code-quality 的 `..HEAD` 則由下述 end 斷言保證等於同一基準。
4. **end 斷言 + 發佈前閘**：於**所有 job 結束、彙整前**再讀一次 `HEAD`；若 ≠ `HEAD_AT_START`
   → 整個批次標為**無效**（非零退出，Summary 加 `BATCH INVALID: HEAD moved during run
   (<a>→<b>) — rerun`），且**不進入發佈**（跳過 §4.3 步驟 3、不寫任何 canonical 檔/manifest），
   要求 caller 重跑。**只有 `HEAD_AT_START` 自始至終不變才發佈**，故 spec-compliance 的釘住終點、
   code-quality 的 `..HEAD`、與基準三者必為同一 commit，達成 §5.3 的「同一 snapshot」。
...
故採務實組合：(a) spec-compliance 已用 sentinel 完全釘住；(b) code-quality 的 `review --base`
終點仍是 `HEAD`，由 caller 契約「阻塞呼叫期間不得推進 HEAD」+ 本斷言（擋最終漂移）覆蓋常見
情形。

**為何無法在約束內完全消除（誠實的限制聲明，非「可接受風險」的搪塞）**：HEAD 是
**repo 全域**狀態，會推進它的是 implementer 的提交或任何外部 git 操作——**不是 read-only 的
review 批次**。因此「repo 級 review 批次鎖」**無效**：真正的 HEAD 變動者不會去搶那個鎖。能
真正消除「瞬態改走又改回」的，只有把 code-quality 釘到固定 ref，而其兩條路徑——
**worktree 隔離**與**改 companion/dispatch.sh 收顯式 end SHA**——**都被本專案的硬性約束
排除**（見上）。故在現行約束下，此瞬態漂移是**約束所迫的已知限制**（blocking-by-constraint），
而非可隨手消除的疏漏。**要消除它，必須由專案層級決定放寬其中一個約束**（允許 worktree、
或允許動 companion/dispatch.sh）。在未放寬前，本設計以 caller 契約 + 最終漂移斷言把風險限縮到
「外部行為者在數秒批次窗內改走又改回 HEAD」的對抗性窄邊界，並在此明確標記為已知限制。

### 4.7 Canonical Reader API（durable 證據的唯一受支援消費方式）

§4.3 的雜湊驗證讀者協定是**破壞性的讀者契約**，必須明定其適用範圍與強制方式，否則直接掃
`*.output.md` 的消費者會繞過驗證、讀到被取代或不同 generation 的檔。

**消費模型盤點（已查證現況）**：

- 三個技能的 review 迴圈**只消費 wrapper 的 stdout**（§4.4 的全文 + Summary），**不讀** durable
  檔——round loop 的判斷一律來自 stdout 的 verdict 行。
- `.claude/superpowers/review/` 是本設計**新增**的目錄；**現況全 repo 無任何讀取該目錄或
  `*.output.md` 的程式**（已 grep `skills/`、`scripts/` 確認）。故**沒有需要遷移的既有讀者**；
  durable 檔僅作**稽核/證據軌跡**。

**唯一受支援的 durable 證據讀法（任何現在或未來的 consumer/稽核/工具都必須遵循）**：

1. 以 `<stage>.<round>.manifest.md` 為入口；**檢查其 `schema-version`**：
   - 版本**可理解** → 繼續；
   - 版本**不可理解或 manifest 不存在** → **fail closed**：視該輪為**不完整、不可信**，**不**
     接受任何 `*.output.md` 為有效證據（寧可拒絕，不可猜測）。
2. 對 manifest 列出的每個 output 檔做 **sha256 雜湊驗證**（§4.3 讀者協定）；任一不符 → 撞上並行
   republish → **重讀重試**，直到取得單一一致 generation 才接受。
3. **直接掃 `*.output.md`（不經 manifest + 雜湊驗證）不是受支援的 API**，**不得**被當成「完整
   有效證據」的判定方式——這類存取最多只能視為「未驗證的原始檔」。

**強制與相容**：`schema-version` 讓未來格式演進時，舊讀者遇到不認得的版本能 fail closed 而非
誤讀。本設計不新增任何 durable 檔讀者（三技能讀 stdout）；**若日後新增任何 durable 證據讀者，
必須走本 §4.7 協定**，並在其測試中涵蓋「直接掃 output 不被當完整證據」「未知 schema-version
fail closed」「雜湊不符重試」三項（§10）。
...
**HEAD 由引擎唯一擷取（§4.6）**：review-impl.sh **不自行 `git rev-parse HEAD`**。
spec-compliance 以 **sentinel** `--set TASK_HEAD=@HEAD_AT_START@` 登記；引擎在取鎖後擷取
單一 `HEAD_AT_START`、並把 argv 中的 `@HEAD_AT_START@` 替換成該 SHA，使 spec-compliance 的
`git diff` 釘成固定的 `[TASK_BASE]..[HEAD_AT_START]` commit pair。code-quality 的
`review --base` 在 companion 內取 `..HEAD`，無參數可釘終點，但由 §4.6 的 end 斷言保證
「批次期間 HEAD == HEAD_AT_START」，故其 `HEAD` 等同同一基準。兩者因此針對**同一個
不可變 commit pair**，且不存在 wrapper/引擎雙擷取的競態。
...
1. **同一 snapshot**：同一次 `review-impl.sh` 呼叫中，引擎在取鎖後**只擷取一次**
   `HEAD_AT_START`（§4.6），spec-compliance 經 sentinel 釘住 `git diff
   <TASK_BASE>..<HEAD_AT_START>`、code-quality 的 `review --base <TASK_BASE>` 在 companion
   內取 `..HEAD`；由 §4.6 的 end 斷言保證批次期間 `HEAD == HEAD_AT_START`（漂移即整批無效、
   不發佈），故兩者必針對**同一個不可變 commit pair**，且無 wrapper/引擎雙擷取競態。
...
- **呼叫端只消費 wrapper 的 stdout，不直接讀 durable 檔**：三個 SKILL.md 的 round loop 一律以
  §4.4 的 stdout（全文 + Summary）為判斷依據，**不得**直接掃 `.claude/superpowers/review/` 下的
  `*.output.md` 來判定輪次完整性。durable 檔是稽核軌跡；若任何工具要消費它，**必須**走 §4.7 的
  Canonical Reader API（manifest + schema-version + sha256 驗證）。
...
- **核心發佈不變式：manifest ⟺ 完整有效輪**：只有「完成批次」（HEAD 穩定且無 ERROR）才
  發佈 canonical + 寫 `<stage>.<round>.manifest.md`（含 per-job verdict）；ERROR/無效/中斷/
  發佈失敗一律不寫 manifest、不覆寫前一完整輪（§4.3 不變式與步驟 3）。manifest 存在即代表
  該輪每個 reviewer 都有真實結論、絕不含工具失敗。
- **同一 `<label>.<round>.output.md` 已存在（同輪重跑）**：以「先備份既有整組 →（**完成批次
  才**）原子 `mv` + 寫新 manifest；非完成或失敗則整組還原」處理，最終檔永遠是某一次完整
  輸出，且重跑為 ERROR/失敗時不毀掉前一次完整證據（§4.3 不變式）。
...
- **同輪重跑發佈失敗保前一輪**：先成功發佈一輪（有 canonical + manifest），再對同輪重跑注入
  某個 output `mv` 失敗，驗證**前一個有效輪的檔與 manifest 原封還原**、移除已 `mv` 的部分新檔、
  不出現「新 manifest 配半套新檔」或「舊 manifest 配缺檔」的混合態、非零退出（§4.3 步驟 3
  `rollback()`）。
- **manifest 原子提交**：模擬 manifest 寫入損毀/中途失敗，驗證最終目錄**不會出現半寫/損毀的
  `<stage>.<round>.manifest.md`**（manifest 先在 staging 寫好驗證、再以原子 rename 提交）；
  rename 提交失敗 → 走 `rollback()`、舊 manifest 保留、該輪不被認證（§4.3 步驟 3）。
- **發佈臨界區中斷還原**：在「備份已 copy 入 staging、manifest 尚未提交」時送 INT/TERM，驗證
  phase-aware shutdown **先 `rollback()` 還原前一完整輪、再清 staging**——備份不會連同 staging
  被刪、durable 證據保持上一完整輪（§4.3 步驟 1 phase-aware shutdown）。
- **republish canonical 路徑永不缺檔**：對已存在完整輪做同輪重跑，全程輪詢既有
  `<label>.<round>.output.md` 路徑，驗證**任一時刻該路徑都存在**（copy-備份 + 原子取代，無
  「舊檔移走、新檔未到」的缺檔視窗）（§4.3 步驟 3）。
- **雜湊驗證讀者協定（並行 republish 一致）**：模擬讀者持舊 manifest，在 republish 把某 output
  原子換成新內容後讀該檔 → 雜湊與舊 manifest 不符 → 讀者**重讀 manifest+outputs 重試** → 取得
  與新 manifest 全數一致的 generation；驗證讀者**永不接受「新 output 配舊 manifest」的混合集**
  （§4.3 / §4.7）。manifest 每筆含正確 sha256。
- **manifest schema-version fail closed**：manifest 含 `schema-version`；讀者遇**未知版本**或
  **manifest 不存在** → fail closed、不接受任何 `*.output.md` 為完整證據（§4.7）。
- **直接掃 output 不被當完整證據**：驗證「未經 manifest + 雜湊驗證、直接掃 `*.output.md`」**不**
  被視為完整有效證據集；唯一受支援的判定是 manifest 入口 + 雜湊驗證（§4.7）。
- **ERROR 後整批重跑**：驗證 caller 契約以**相同完整 job set** 重跑整個 wrapper（review-plan
  須帶相同 `--task`/`--coverage`），引擎不提供 per-job 局部重跑/合併（§4.5）。
- **ERROR 不產生被接受的 manifest（核心不變式）**：批次含一個 ERROR job 時，驗證
  **不寫 `<stage>.<round>.manifest.md`**、依讀者規則該輪**不被當完整有效集**；若是同輪重跑，
  前一個完整輪的檔與 manifest **原封保留**（未被 ERROR 結果覆寫）；所有結果（含 ERROR 與其餘
  reviewer 的 verdict）仍在 stdout、批次非零退出（§4.3 不變式/步驟 3、§4.5）。
- **findings 是完成批次**：所有 job 皆 `Issues Found`/`needs-attention`/prose（無 ERROR）→
  **發佈 + 寫 manifest**（完成）、批次退出 0（§4.3 不變式）。
- **瞬態 HEAD 漂移為已記載限制**：start==end 但中途改走又改回 → 斷言不報（記載限制，非
  bug）；最終仍不同 → 報 `BATCH INVALID`（§4.6）。
- **引擎單一 HEAD 擷取 + sentinel 注入（無雙擷取競態）**：驗證 wrapper 不呼叫
  `git rev-parse`；在 `batch_add` 登記 `--set TASK_HEAD=@HEAD_AT_START@` 後、`batch_run`
  取鎖擷取 `HEAD_AT_START` 並把 argv 的 `@HEAD_AT_START@` 替換為該 SHA；spec-compliance 收到
  的 `TASK_HEAD` **恆等於** `HEAD_AT_START`。模擬「wrapper 進入後、引擎擷取前 HEAD 變動」→
  仍只有單一基準、不會 spec-compliance 釘舊而 code-quality 讀新；批次內 HEAD 再變 → end 斷言
  報 `BATCH INVALID` 不發佈（§4.6 / §5.3）。
...
- **HEAD 凍結＝引擎單一擷取 + sentinel 注入 + 端點斷言**（vs wrapper 與引擎各自 `git
  rev-parse`）：引擎在取鎖後只擷取一次 `HEAD_AT_START`，以 sentinel `@HEAD_AT_START@` 注入
  spec-compliance 的釘住終點，code-quality 的 `..HEAD` 由 end 斷言保證等於同一基準——消除
  wrapper/引擎雙擷取的 TOCTOU，且不需改 dispatch.sh/companion。
...
- **核心發佈不變式：manifest ⟺ 完整有效輪**（vs「ERROR 也發佈 + 寫 manifest」）：只有
  HEAD 穩定且無 ERROR 的批次才發佈 + 寫 manifest，故 durable 證據絕不會把含工具失敗的輪
  認證為完整；ERROR/無效/中斷/發佈失敗都不寫 manifest、不覆寫前一完整輪，結果仍走 stdout
  供即時 caller 重跑。manifest 另記 per-job verdict 供稽核。
...
- **manifest 為原子提交標記 + phase-aware shutdown**（vs 直接 write manifest、trap 無條件
  `rm -rf` staging）：manifest 先在 staging 寫好驗證、再以單一原子 rename 提交，讀者永不見
  半寫/損毀 manifest；備份雖存在 staging，但 shutdown 是 phase-aware——發佈臨界區內中斷會
  **先 `rollback()` 還原再清理**，且臨界區延遲 INT/TERM，故唯一的備份不會連同 staging 被誤刪。
- **HEAD 凍結的約束所迫限制**：spec-compliance 完全釘住；code-quality 終點因「禁用 worktree +
  不改 companion」無法釘住，start/end 斷言只擋最終漂移。瞬態改走又改回是 **repo 全域 HEAD +
  禁用 worktree + dispatch.sh 不變** 三約束交集下**無法在約束內消除**的限制（repo 級 review
  鎖無效，因 HEAD 變動者非 review 批次）——列為**已知限制**並指明唯一出路是專案層級放寬約束
  （見 §4.6 與 §12）。
- **ERROR 復原＝整批重跑**（vs per-job 局部重跑）：manifest 是整批範圍，避免不同 snapshot 結果
  混入同一 manifest；固定 wrapper 本就跑固定 job set，review-plan 重跑須帶相同 `--task`/`--coverage`。
- **並行 republish 下證據一致＝copy-備份 + 原子覆寫 + 雜湊驗證讀者協定**（vs 加 run-id 改檔名
  方案 / 不防護）：copy-備份 + 原子取代消除「缺檔視窗」；manifest 記每個 output 的 sha256、
  讀者比對雜湊不符即重讀重試（§4.3），故並行讀者**絕不接受新舊混合**——在不改使用者指定檔名
  方案、不需 reader 取鎖下達成 generation 一致性。
- **Canonical Reader API + schema-version + 無既有讀者**（§4.7）：已查證 repo 無任何讀 durable 檔的
  程式、SKILL loop 只讀 stdout，故無遷移負擔；明定 manifest 為唯一入口、未知 schema-version 或
  無 manifest 即 fail closed、直接掃 output 不算完整證據——讓未來讀者/格式演進不會悄悄誤讀。
...
### 12.1 約束所迫的已知限制（在現行硬性約束下無法消除，需專案層級決定才能解）

本設計受三條專案硬性約束限定：**(i) 中繼檔名為 `<reviewer>.<round>.output.md`（無 run-id）**、
**(ii) 禁用任何 worktree 隔離**、**(iii) `dispatch.sh` 與 codex companion 不得修改**。下列兩項
是這些約束交集下**無法在約束內完全消除**的限制，已盡力把風險限縮到單一 agent 循序模型外才會
觸發的窄邊界，並在此明確標記（非設計疏漏）：

1. **瞬態 HEAD 漂移（§4.6）**：code-quality 的 `..HEAD` 無法釘到固定 ref（需 (ii) 或 (iii) 放寬），
   start/end 斷言只擋最終漂移。**消除方式**：放寬 (ii)（允許 detached worktree 釘 HEAD）或 (iii)
   （讓 `review` 收顯式 end SHA）。未放寬前列為已知限制。
（原「同輪重跑期間並行讀者短窗不一致」**已在約束內解決**，不再列為限制：見 §4.3 的
**雜湊驗證強制讀者協定**——manifest 記每個 output 的 sha256，讀者比對雜湊、不符即重讀重試，
故並行 republish 下讀者要嘛拿到完整舊 generation、要嘛重試後拿完整新 generation，**絕不接受
新舊混合**；配合 copy-備份 + 原子覆寫消除缺檔視窗。此方案不需改檔名方案 (i)、不需 reader 取鎖。）
