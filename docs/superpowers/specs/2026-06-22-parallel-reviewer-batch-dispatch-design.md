...
`<round>.manifest.md` 內記錄 **原始 label → canonical 檔名** 的對映，確保 manifest 與檔案
...
- `--name <spec>`（選填）：覆寫 `<spec name>`；未給時由各 wrapper 的**主要文件檔名**
  ...
  **`<spec name>` 嚴格驗證（對推導值與 `--name` 覆寫值都套用，於建立目錄/鎖之前）**：
  只允許 `^[A-Za-z0-9._-]+$`；**拒絕**空值、`.`、`..`、含路徑分隔符（`/`、`\`）、或絕對
  路徑樣式的值——避免把 lock/manifest/輸出寫到 review 命名空間外，或讓兩個邏輯 spec
  別名到同一目錄而互相覆寫（這也保護 §9 的 per-`<spec name>` 鎖）。不合法 → fail fast。
...
- **先彙整後發佈順序**：驗證彙整在 `mv` 之前完成（私有暫存於彙整時仍存在），最終檔在
  彙整後才出現（§4.3 步驟順序）。
- **HEAD 漂移偵測 + 不發佈 + temp 清理**：在批次執行中讓 `HEAD` 變動，驗證引擎標
  `BATCH INVALID`、非零退出，**且不產生任何 canonical 檔或 manifest**，**且私有暫存目錄
  被 trap 清掉**（review 命名空間外無殘骸）（§4.3 / §4.6）；HEAD 不動時正常發佈。
...
- **`<spec name>` 驗證**：`--name` 或推導值含 `..`、`/`、`\`、前導 `.`、空值、絕對路徑樣式
  → fail fast；合法值（`[A-Za-z0-9._-]+`）通過，且輸出/鎖/manifest 不逸出 review 命名空間
  （§5）。
