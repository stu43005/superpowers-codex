...
| **單一 job 工具失敗**（無 verdict 行 → `ERROR`） | **非完成 → 不寫 manifest、還原前一完整輪** | 其餘 reviewer 結果照常經 stdout 給 agent；該 job 在 Summary 標 `ERROR`，批次非零退出、caller **重跑**；durable 證據維持上一個完整輪不被污染。 |
...
3. **發佈（全有全無、對同輪重跑可還原，僅在「完成批次」時）**：彙整完成**之後**，才依
   ...
   - **完成判定**：批次未被 §4.6 判為無效（HEAD 自始至終穩定）**且無任何 job 為 ERROR**
     ...
     「非完成處置」。
   - **先備份**（無論完成與否，只要要動最終區）：若該（stage, round）的 canonical 檔/
     `<stage>.<round>.manifest.md` 已存在（同輪重跑），先將**整組**既有檔搬到本次 staging 內
     的備份區（同檔案系統、不是刪除）。
   - **完成批次 → 發佈（manifest 為原子提交標記）**：此為**發佈臨界區**，期間延遲 INT/TERM
     ...
     2. **發佈 output**：把每個 job 的 stdout staging 檔以 **same-filesystem 原子 `mv`** 改名到
...
- **發佈臨界區中斷還原**：在「備份已移入 staging、manifest 尚未提交」時送 INT/TERM，驗證
