...
| **訊號中斷**（INT/TERM，發佈前） | **非完成 → 不發佈** | 走 shutdown 路徑：終止子行程、清暫存、釋鎖。 |
| **發佈步驟失敗**（某 `mv` 失敗） | **非完成 → 還原前一完整輪、不寫 manifest** | 還原備份（見步驟 3）。 |
...
3. **發佈（全有全無、對同輪重跑可還原，僅在「完成批次」時）**：彙整完成**之後**，才依
   ...
   - **先備份**（無論完成與否，只要要動最終區）：若該（stage, round）的 canonical 檔/
     ...
     的備份區（同檔案系統、不是刪除）。
   - **完成批次 → 發佈**：把每個 job 的 stdout staging 檔以 **same-filesystem 原子 `mv`** 改名
     到 `<label>.<round>.output.md`；全部成功後**最後**寫入本輪 **manifest**
     `<stage>.<round>.manifest.md`。manifest 記錄本 wrapper 本輪**每個 label → canonical 檔名 →
     verdict**（皆為非 ERROR 的真實結論），作為「本（stage, round）完整有效」的權威信號。
     讀者僅在對應 manifest 存在、且其列出的檔齊備時，才把該輪視為完整有效集。發佈與 manifest
     全部成功 → 丟棄備份。
   - **非完成處置（ERROR / 無效 / 發佈中途 `mv` 失敗）→ 整輪還原、不寫 manifest**：把備份區的
     **整組舊檔與舊 manifest 原封還原**，使前一個已 commit 的完整輪**完全保持原狀**（durable
     證據不被工具失敗或半套結果污染）；**不寫新 manifest**；以非零退出回報。**絕不**留下
     「新 manifest 指向缺檔」「舊 manifest 配被覆寫的 ERROR 檔」或「manifest 認證含 ERROR 的
     輪」之類的混合態。非完成批次的全部結果（含 ERROR 項）仍經 stdout 回傳，caller 依
     §4.5 重跑。（HEAD 無效與訊號中斷屬非完成的子情形：前者於彙整前即判定、後者由
     shutdown 處理，兩者都不發佈、不寫 manifest，與上述一致。）
